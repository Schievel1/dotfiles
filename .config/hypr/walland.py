...
def set_wallpaper(image_path, backend='hyprpaper', backend_args=''):
    ...
    if backend == 'hyprpaper':
        ...
        # mod by pascal: only set for DVI-D-2
        subprocess.Popen(shlex.split(f'hyprctl hyprpaper wallpaper "DVI-D-2,{image_path}" {backend_args}'), stdout=subprocess.PIPE).wait()
